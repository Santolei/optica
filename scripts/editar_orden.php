<script>
    ...
    $('#add_patologia').select2({
        dropdownParent: $('#modal_edit_new_order2')
    });
    // Agrego los option de las vendedores y de los pacientes
    $("#add_vendedor").load('consultas/vendedores_ajax.php');
    $("#add_patologia").load('consultas/edit_patologias_ajax.php');
    $("#add_patologia option[value=<?php echo($orden['patologia']) ?>").attr('selected','selected');
    ...
    $('#datos_paciente').on('click', function(){
        $('#modal_edit_new_order').modal('hide');
    ...
    //Funcion que pasa el string que traigo de la BD a fecha
    function toDate(dateStr) {
      var parts = dateStr.split("-")
      return new Date(parts[2], parts[1] - 1, parts[0]);
    }

    var fecha_receta = toDate("<?php echo $orden['fecha_receta'] ?>");

    var now = new Date();

    var day = ("0" + fecha_receta.getDate()).slice(-2);
    var month = ("0" + (fecha_receta.getMonth() + 1)).slice(-2);

    var fecha_receta_final = fecha_receta.getFullYear()+"-"+(month)+"-"+(day) ;

    
    
    

    $('#add_fecha_receta').val(fecha_receta_final);
    
    $('#add_tratamiento').select2({
        dropdownParent: $('#modal_edit_new_order3')
    });
    ...
    $('#datos_receta').on('click', function(){
        ...
        $('#modal_edit_new_order2').modal('hide');
    });
    ...
    $('#datos_armazon').on('click', function(){
        ...
        $('#modal_edit_new_order4').modal('hide');
    });
